...
class Model(object):
    def __init__(self, checkpoint_dir, log_dir):
        ...
        if hyp.do_eval_recall:
            ...
            # if hyp.do_emb2D:
            #     F = 21
            #     self.eval_dicts["eval"]['pool2D_e'] = DoublePool(self.pool_size)
            #     self.eval_dicts["eval"]['pool2D_g'] = DoublePool(self.pool_size)
            #     self.eval_dicts["eval"]['precision2D'] = np.nan*np.array([0.0, 0.0, 0.0], np.float32)
            #     self.eval_dicts["eval"]['neighbors2D'] = np.zeros((F*10, F*11, 3), np.float32)
        ...
        # we do not create them here because we don't want an empty tfrecord file if we don't test/val.
        # self.all_writers = {}
        # self.all_writers['train'] = SummaryWriter(self.log_dir + '/train', max_queue = MAX_QUEUE)
        # self.all_writers['val'] = SummaryWriter(self.log_dir + '/val', max_queue = MAX_QUEUE)
        # self.all_writers['test'] = SummaryWriter(self.log_dir + '/test', max_queue = MAX_QUEUE)
