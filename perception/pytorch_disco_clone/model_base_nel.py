                    ...
                    for num_epoch in range(maxm.max_epochs):
                        if maxm.hard_moc:
                            ...
                            for j_ in range(iterEpoch) :
                                ...
                                # ranks = torch.where(torch.argsort(logits)==0)[1]
                                # num_logits = ranks.shape[0]
                                # ns = "retrieval"
                                # for recall in [100,500,1000]:
                                #     average = torch.sum(ranks<recall).float()/float(num_logits)
                                #     summ_writer.summ_scalar(ns + 'precision3D_{:02d}_avg'.format(recall),average)
