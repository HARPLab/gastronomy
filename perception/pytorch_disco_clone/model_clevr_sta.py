...
class ClevrStaNet(nn.Module):
    ...
    def forward(self, feed):
        ...
        if hyp.do_pixor_det or hyp.do_det:
                ...
                # print("total time taken",time.time() - start_pixor_det)
        # if hyp.gt_rotate_combinations:
        #     unpRs_comb = torch.mean(unpRs,dim=1,keepdim=False)
        #     occRs_comb = torch.max(occRs,dim=1,keepdim=False).values

        #     unpX1s_eg, _ = nlu.create_object_tensors([unpXs[:,1]], None, gt_boxesX1Unp_end, scores,[BOX_SIZE*2,BOX_SIZE*2,BOX_SIZE*2])
        #     occX1s_eg, _ = nlu.create_object_tensors([occXs[:,1]], None, gt_boxesX1Unp_end, scores,[BOX_SIZE*2,BOX_SIZE*2,BOX_SIZE*2],occs=True)
        #     unps_visX1s_eg = utils_improc.get_unps_vis(unpX1s_eg.unsqueeze(1), occX1s_eg.unsqueeze(1))
        ...
        #     unp_visRs_eg_rotated = self.mbr_unpr.rotateTensor(unpRs_eg)
        #     occ_visRs_eg_rotated = self.mbr_unpr.rotateTensor(occRs_eg,interpolation="nearestNeighbor")
        #     unps_visRs_eg_rotated = utils_improc.get_unps_vis(unp_visRs_eg_rotated, occ_visRs_eg_rotated,dim=3)

        #     unp_visRs_eg_rotated_unstacked = torch.unbind(unps_visRs_eg_rotated,dim=1)
        #     # st()

        #     inp_best_quant = torch.cat(unp_visRs_eg_rotated_unstacked,dim=2)
        #     # st()
        #     summ_writer.summ_rgb('selected_rotations_new/all_rots',inp_best_quant)
            # st()
        if hyp.create_example_dict:
