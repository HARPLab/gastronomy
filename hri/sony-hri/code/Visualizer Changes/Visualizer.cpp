	...
	const std::vector<std::pair<std::string, double> >& au_occurences)
{
	if (au_intensities.size() > 0 || au_occurences.size() > 0)
	{
        ...
        //Similar, but for simpler goal of individual AUs
        ...
        // Set the neediness and interruptibility
        ...
        for (auto& cla : aus)
        {
            ...
            need = need + (intensity / 18.0);
            
            int offset = MARGIN_Y + idx * (AU_TRACKBAR_HEIGHT + 10);
            std::ostringstream cla_i;
            cla_i << std::setprecision(2) << std::setw(4) << std::fixed << intensity;
            ...
            if (present)
            {
                cv::putText(action_units_image, cla_i.str(), cv::Point(160, offset + 10), cv::FONT_HERSHEY_SIMPLEX, 0.3, CV_RGB(0, 100, 0), 1, cv::LINE_AA);
        ...
        //need = .4;//intensities_map[AUS_DESCRIPTION[4]] + .5*intensities_map[AUS_DESCRIPTION[5]] + .5*intensities_map[AUS_DESCRIPTION[0]];
        
        interruptibility = 1 - need;//(3.0 - (intensities_map[AUS_DESCRIPTION[4]] + .5*intensities_map[AUS_DESCRIPTION[7]] + .5*intensities_map[AUS_DESCRIPTION[7]])) / 3.0;
...
char Visualizer::ShowObservation()
{
    ...
    if (vis_aus && !action_units_image.empty() && !classifier_image.empty()) {
    	cv::imshow("Status Classifiers", classifier_image);
    	ovservation_shown = true;
    }
