...
class MagneticCalibration:
    ...
    def saveData(self, filename, starting_location, vision_object_location, magnet_object_location, robot_final_location):
        ...
        np.savez(filename, magnetic_data=my_magnetic_data, robot_state_data=my_robot_state_data, gripper_state_data=my_gripper_state_data, starting_location=starting_location, vision_object_location=vision_object_location, magnet_object_location=magnet_object_location, robot_final_location=robot_final_location)
...
if __name__ == '__main__':
    ...
    constant_x = np.random.rand(1)[0] * 0.08 + 0.47
    constant_y = -0.025
    constant_z = np.random.rand(1)[0] * 0.015 + 0.082
    if constant_x > 0.51:
        x_scan_dist = -0.05
    else:
        x_scan_dist = 0.05
    ...
    desired_object_pose = RigidTransform(
        rotation=np.array([[1, 0,  0],
                           [0,  -1,  0],
                           [0, 0,  -1]]), 
        translation=np.array([constant_x+x_scan_dist, constant_y, constant_z]),
         from_frame='franka_tool', to_frame='world')
    ...
    if args.filename is not None:
        magnetic_calibration.saveData(args.filename+'_x', starting_location, vision_object_location, max_location, fa.get_pose().translation)
    ...
    desired_object_pose = RigidTransform(
        rotation=np.array([[1, 0,  0],
                           [0,  -1,  0],
                           [0, 0,  -1]]), 
        translation=np.array([max_location[0], max_location[1], max_location[2]+z_scan_dist]),
         from_frame='franka_tool', to_frame='world')
    ...
    if args.filename is not None:
            magnetic_calibration.saveData(args.filename+'_z', starting_location, vision_object_location, max_location, fa.get_pose().translation)
    ...
    cv2.imwrite(args.filename+'_z.png', cropped_image)
