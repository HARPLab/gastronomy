...
from action_relation.model.losses import TripletLoss
...
class VoxelRelationTrainer(BaseVAETrainer):
    def __init__(self, config):
        ...
        self.triplet_loss = TripletLoss(5.0)
    ...
    def print_train_update_to_console(self, current_epoch, num_epochs,
        ...
        if train_step_count % args.print_freq_iters == 0:
            if args.loss_type == 'regr' and args.use_contrastive_loss:
        ...
        for e in range(num_epochs):
            ...
            for batch_idx in range(num_batches):
                ...
                while len(batch_data) < batch_size and data_idx < len(iter_order):
                    data = dataloader.get_train_data_at_idx(
                        iter_order[data_idx], train=train)
                    if train and args.use_contrastive_loss:
                        contrastive_data = dataloader.get_contrastive_data_at_idx(
                            iter_order[data_idx], train=train)

                    if args.octree_0_multi_thread:
                        assert data['voxels'] is None
                        data['voxels'] = voxel_tensor_list[b]

                    batch_data.append(data)
                    if train and args.use_contrastive_loss:
                        batch_contrastive_data.append(contrastive_data)
                    data_idx = data_idx + 1
                    b = b + 1
                ...
                if args.use_contrastive_loss:
                    x_contrastive_dict = self.process_contrastive_raw_batch_data(
                        batch_contrastive_data,
                    )
                    contrastive_x_tensor_dict = {}
                    for contrastive_key, contrastive_x_dict in x_contrastive_dict.items():
                        contrastive_x_tensor_dict[contrastive_key] = \
                            self.collate_batch_data_to_tensors(contrastive_x_dict)
                else:
                    contrastive_x_tensor_dict = None
                ...
                batch_result_dict = self.run_model_on_batch(
                    x_tensor_dict, 
                    batch_size, 
                    contrastive_x_tensor_dict=contrastive_x_tensor_dict,
                    train=train, 
                    save_preds=True)
