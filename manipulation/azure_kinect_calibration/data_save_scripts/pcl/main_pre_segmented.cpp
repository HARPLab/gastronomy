...
//                           "/try_8_Nov_30_2019_12_01_AM/temp_2/final_segmented_pcl.pcd";
    ...
    // Extract different clusters in the point cloud?
    ...
    int num_objects = 3;
    float cluster_tolerance = 0.01;
    while (cluster_indices.size() != num_objects && cluster_tolerance >= 0.003) {
        // Clear the indices
        ...
        ec.setInputCloud(projected_cloud);
        ec.extract(cluster_indices);

        cluster_tolerance -= 0.0005;
    }
    std::cout << "Found " << cluster_indices.size() << " clusters, with tolerance: " << cluster_tolerance << std::endl;
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it) {
        ...
        for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit) {
            cloud_cluster->points.push_back(projected_cloud->points[*pit]);
        }
        ...
        // Project points onto the table plane to complete the object.
//        pcl::PointCloud<pcl::PointXYZRGB>::Ptr proj_cloud_cluster =
//            CreatePointCloudWithPlaneProjection(cloud_cluster, table_plane_vector, true);

        SaveCloud(writer, pcl_dir + "/cloud_cluster_" + std::to_string(j) + ".pcd", cloud_cluster);
        // SaveCloud(writer, pcl_dir + "/complete_cloud_cluster_" + std::to_string(j) + ".pcd", proj_cloud_cluster);

        j++;
    }
    if (j == 0) {
        std::cout << "ERROR: did not extract any objects from the point cloud data!!" << std::endl;
    }
