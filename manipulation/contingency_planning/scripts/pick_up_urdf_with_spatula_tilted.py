...
from spatula_policies import PickUpURDFTiltedPolicy
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', '-dd', type=str, default='urdf_data/pick_up/spatula_tilted/')
    ...
    scene.add_asset('urdf0', urdf, urdf_pose, collision_filter=4)
    if use_pan:
    ...
    if args.inputs is None:
        ...
        x_y_theta_dist_tilts = input_data['X']
        print(x_y_theta_dist_tilts.shape)
        ...
        if len(nonzero_trials[0]) == 0:
            ...
            x_y_theta_dist_tilts = x_y_theta_dist_tilts[nonzero_trials[0],:]
        ...
        if settling_t_step == 150:
            ...
            if t_step == settling_t_step+1:
                ...
                for env_idx, env_ptr in enumerate(scene.env_ptrs):
                    ...
                    urdf_ah = scene.ah_map[env_idx]['urdf0']
                    pre_push_urdf_pose[simulation_num * n_envs + env_idx] = transform_to_np(urdf.get_rb_transforms(env_ptr, urdf_ah)[0])
                ...
                for env_idx, env_ptr in enumerate(scene.env_ptrs):
                    ...
                    urdf_ah = scene.ah_map[env_idx]['urdf0']
                    push_urdf_pose[simulation_num * n_envs + env_idx] = transform_to_np(urdf.get_rb_transforms(env_ptr, urdf_ah)[0])
            elif t_step == settling_t_step+599:
                for env_idx, env_ptr in enumerate(scene.env_ptrs):
                    urdf_ah = scene.ah_map[env_idx]['urdf0']
                    post_pick_up_urdf_pose[simulation_num * n_envs + env_idx] = transform_to_np(urdf.get_rb_transforms(env_ptr, urdf_ah)[0])
            elif t_step == settling_t_step+949:
                for env_idx, env_ptr in enumerate(scene.env_ptrs):
                    urdf_ah = scene.ah_map[env_idx]['urdf0']
