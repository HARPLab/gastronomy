...
if __name__ == "__main__":
    ...
    parser.add_argument('--visualize_block_num', '-vbn', type=int, default=0)
    parser.add_argument('--sampling_method', '-s', type=int, default=0)
    args = parser.parse_args()

    data = np.load('baseline/pick_up/good_grasp_data.npy')
    x_y_thetas = np.load('data/franka_fingers_inputs.npy')

    if args.baseline_type == 'baseline_same_friction':
        baseline_data = np.load('baseline_same_friction/pick_up/grasp_data.npy')
    elif args.baseline_type == 'baseline_same_mass':
        baseline_data = np.load('baseline_same_mass/pick_up/grasp_data.npy')
