...
if __name__ == "__main__":
    ...
    if args.num_blocks == -1:
        num_blocks_to_use = num_blocks
    else:
        num_blocks_to_use = args.num_blocks
    ...
    for current_block_id in range(num_blocks):
        ...
        if args.baseline_type == 'baseline':
            if args.num_blocks == -1:
                ...
                if current_block_id == 0:
                    baseline_data = data[:,1:(num_blocks_to_use+1)]
                elif current_block_id >= num_blocks_to_use:
                    baseline_data = data[:,:num_blocks_to_use]
                else:
                    baseline_data = np.hstack((data[:,:current_block_id].reshape(num_skills,-1), data[:,(num_blocks_to_use+1):].reshape(num_skills,-1)))
        else: 
            if args.num_blocks == -1:
                baseline_data = baseline_data[:num_skills,:]
            else:
                baseline_data = baseline_data[:num_skills,:num_blocks_to_use]
        ...
        for i in range(args.num_trials):
            ...
            if args.visualize_block_num >= 0:
                ...
                ax.scatter(x_y_thetas[skill_num,0], x_y_thetas[skill_num,1], c='red');
                if i == 0:
