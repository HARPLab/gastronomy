...
class PickUpBlockTongsOverheadPolicy(Policy):
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 50:
            ...
            pre_grasp_offset = [x_y_theta[0], 0.145, -x_y_theta[1]]
...
class PickUpBlockTongsSidePolicy(Policy):
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 50:
            ...
            if(theta > 0):
                ...
                backoff_distance = x_y_theta_dist_tilt[1] + 0.145*np.sin(np.abs(tilt_angle))
            else:
                displacement = x_y_theta_dist_tilt[3]
                tilt_angle = -x_y_theta_dist_tilt[4]
                backoff_distance = -x_y_theta_dist_tilt[1] - 0.145*np.sin(np.abs(tilt_angle))

            pre_grasp_offset = [x_y_theta_dist_tilt[0], backoff_distance, 0.0]

            push_offset = [x_y_theta_dist_tilt[0], backoff_distance + displacement, 0.0]

            height = 0.145*np.cos(np.abs(tilt_angle)) + 0.013
...
class PickUpURDFTongsOverheadPolicy(Policy):
...
class PickUpURDFTongsSidePolicy(Policy):
    ...
    def __init__(self, franka, franka_name, urdf, urdf_name, n_envs, x_y_theta_dist_tilts):
        ...
        self._time_horizon = self._settling_t_step + 1050
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 0:
            ...
            if t_step == self._settling_t_step + 600:
                self._franka.close_grippers(env_ptr, ah)

            if t_step > self._settling_t_step + 650 and t_step <= self._settling_t_step + 750:
                self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._post_grasp_trajectory[env_index][t_step-(self._settling_t_step + 651)])

            if t_step == self._settling_t_step + 850:
                self._franka.open_grippers(env_ptr, ah)

            if t_step > self._settling_t_step + 900 and t_step <= self._settling_t_step + 1000:
                self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._post_release_trajectory[env_index][t_step-(self._settling_t_step + 901)])