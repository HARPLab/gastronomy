...
from tong_policies import PickUpURDFTongsSidePolicy
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', '-dd', type=str, default='urdf_data/pick_up/tongs_side/')
    ...
    policy = PickUpURDFTongsSidePolicy(franka, 'franka0', urdf, 'urdf0', n_envs, x_y_theta_dist_tilts)
    ...
    def save_urdf_pose(scene, t_step, t_sim, simulation_num):
        ...
        if settling_t_step == 150:
            ...
            elif t_step == settling_t_step+650:
                for env_idx, env_ptr in enumerate(scene.env_ptrs):
                    franka_ah = scene.ah_map[env_idx]['franka0']
            ...
            elif t_step == settling_t_step+1049:
                for env_idx, env_ptr in enumerate(scene.env_ptrs):
                    urdf_ah = scene.ah_map[env_idx]['urdf0']
    ...
    save_file_name = args.data_dir+urdf_name+'_tongs_side.npz'
