...
if __name__ == "__main__":
    ...
    for current_block_id in range(101):
        ...
        #print(np.sum(data[:,current_block_id]))
        #if np.sum(data[:,current_block_id]) > 0:
        skills_tested = []

        initial_block_affinities = np.ones(101)
        initial_block_probabilities = np.ones(101)
        division_factor = 0.0
        ...
        #     while baseline_data[random_skill_num,random_block_num] == 0:
        #         random_block_num = pick_random_block_num(initial_block_affinities)
        #         random_skill_num = np.random.randint(2000)
        ...
        # plt.bar(np.arange(101),initial_block_affinities)
        # plt.show()

        num_successes_each_block[current_block_id] = num_successes
        # else:
        #     num_successes_each_block[current_block_id] = 0
        #     first_success_each_block[current_block_id] = -1
        #print(num_successes)
        #print(initial_block_affinities)
        #print(np.argmax(initial_block_affinities))


    # print(str(np.mean(first_success_each_block[np.nonzero(first_success_each_block != -1)])) + ' +- ' + str(np.std(first_success_each_block[np.nonzero(first_success_each_block)])))
