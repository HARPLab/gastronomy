    ...
    for suffix_idx2 in range(suffix_idx1, num_skills):
        ...
        for i in range(lift_data.shape[0]):
            for j in range(lift_data.shape[1]):
                if(lift_data[i,j]):
                    success_mat[i,:] += lift_data[:,j].reshape(-1) / num_successes
                else:
                    failure_mat[i,:] += lift_data[:,j].reshape(-1) / num_successes
        ...
        if (suffix_idx1 == suffix_idx2):
            np.savez('same_block_data/contingency_data/' + suffix1 + '_contingency_data_pick_up_only.npz', X=Xs, success_Y=success_Ys, failure_Y=failure_Ys)
        else:
            np.savez('same_block_data/contingency_data/' + suffix1 + '_' + suffix2 + '_contingency_data_pick_up_only.npz', X=Xs, success_Y=success_Ys, failure_Y=failure_Ys)
