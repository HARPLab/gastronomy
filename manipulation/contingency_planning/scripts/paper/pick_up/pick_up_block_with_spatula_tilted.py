...
from spatula_policies import PickUpBlockTiltedPolicy
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', '-dd', type=str, default='same_block_data/')
    ...
    x_y_z_theta_tilt_dists = get_random_x_y_z_theta_tilt_dists(num_iterations)
    x_y_z_rx_ry_rz = get_random_x_y_z_rx_ry_rz(num_iterations)

    policy = PickUpBlockTiltedPolicy(franka, 'franka0', block, 'block0', n_envs, x_y_z_theta_tilt_dists, x_y_z_rx_ry_rz)
    ...
    post_push_contact_forces = np.zeros((num_iterations,3))
    post_push_block_pose = np.zeros((num_iterations,7))
    ...
    def save_block_pose(scene, t_step, t_sim, simulation_num):
        ...
        if t_step == 49:
            ...
            for env_idx, env_ptr in enumerate(scene.env_ptrs):
                franka_ah = scene.ah_map[env_idx]['franka0']
                post_push_contact_forces[simulation_num * n_envs + env_idx] = franka.get_ee_ct_forces(env_ptr, franka_ah)

                block_ah = scene.ah_map[env_idx]['block0']
                post_push_block_pose[simulation_num * n_envs + env_idx] = transform_to_np(block.get_rb_transforms(env_ptr, block_ah)[1])
        elif t_step == 449:
            for env_idx, env_ptr in enumerate(scene.env_ptrs):
                franka_ah = scene.ah_map[env_idx]['franka0']
    ...
    save_file_name = args.data_dir+'block'+str(args.block_num)+'_pick_up_block_with_spatula_tilted.npz'
    np.savez(save_file_name, x_y_z_theta_tilt_dists=x_y_z_theta_tilt_dists, 
                             ...
                             post_push_contact_forces=post_push_contact_forces,
                             post_push_block_pose=post_push_block_pose,
