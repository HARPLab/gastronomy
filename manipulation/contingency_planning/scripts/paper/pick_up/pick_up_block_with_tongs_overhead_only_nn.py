...
from tong_policies import PickUpBlockOverheadOnlyPolicy
...
if __name__ == "__main__":
    ...
    parser.add_argument('--grasp_nn', type=str, default='same_block_data/tongs_overhead_pick_up_only_trained_model.h5')
    parser.add_argument('--failure_contingency_nn', type=str, default='same_block_data/tongs_overhead_pick_up_only_failure_contingency_model.h5')
    parser.add_argument('--success_contingency_nn', type=str, default='same_block_data/tongs_overhead_pick_up_only_success_contingency_model.h5')
    ...
    parser.add_argument('--data_dir', '-dd', type=str, default='same_block_data/pick_up_only/')
    ...
    failure_contingency_nn_model = load_model(args.failure_contingency_nn)
    success_contingency_nn_model = load_model(args.success_contingency_nn)
    ...
    policy = PickUpBlockOverheadOnlyPolicy(franka, 'franka0', block, 'block0', n_envs, resampled_points)
    ...
    for simulation_num in range(num_simulations):
        ...
        if((post_grasp_block_pose[simulation_num,1] - initial_block_pose[simulation_num,1]) > 0.1):
            # print(simulation_num)
            # break
