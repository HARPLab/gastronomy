...
class PickUpURDFTiltedPolicy(Policy):
    ...
    def __init__(self, franka, franka_name, urdf, urdf_name, n_envs, x_y_theta_tilt_dists):
        ...
        self._time_horizon = self._settling_t_step + 400
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 0:
            ...
            if t_step == self._settling_t_step + 100:
...
class PickUpBlockFlatPolicy(Policy):
    ...
    def __init__(self, franka, franka_name, block, block_name, n_envs, x_y_z_theta_dists, x_y_z_rx_ry_rz):
        self._franka = franka
        self._franka_name = franka_name
        self._block = block
        self._block_name = block_name
        self._n_envs = n_envs
        self._x_y_z_theta_dists = x_y_z_theta_dists
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 50:
            ...
            x_y_z_theta_dist = self._x_y_z_theta_dists[self._simulation_num * self._n_envs + env_index]
            x_y_z_rx_ry_rz = self._x_y_z_rx_ry_rz[self._simulation_num * self._n_envs + env_index]
            
            random_trans = [x_y_z_rx_ry_rz[0], x_y_z_rx_ry_rz[2], -x_y_z_rx_ry_rz[1]]

            theta = x_y_z_theta_dist[3]
            displacement = x_y_z_theta_dist[4]
            backoff_distance = -x_y_z_theta_dist[1] - 0.3

            pre_grasp_offset = [x_y_z_theta_dist[0], backoff_distance, 0.0]

            push_offset = [x_y_z_theta_dist[0], backoff_distance + displacement, 0.0]

            height = -0.03

            self._pre_grasp_transforms[env_index] = RigidTransform_to_transform(transform_to_RigidTransform(gymapi.Transform(
                p=block2_transform.p + np_to_vec3([0.0, 0.2, 0.0]),
                r=self._ee_transforms[env_index].r * rpy_to_quat((0, 0, x_y_z_theta_dist[3]))), 
                from_frame='ee_frame',to_frame='ee_frame') * RigidTransform(
            ...
            self._push_transforms[env_index] = RigidTransform_to_transform(transform_to_RigidTransform(gymapi.Transform(
                p=block2_transform.p + np_to_vec3([0.0, height, 0.0]),
                r=self._ee_transforms[env_index].r * rpy_to_quat((0, 0, x_y_z_theta_dist[3]))), 
                from_frame='ee_frame',to_frame='ee_frame') * RigidTransform(
                translation=push_offset, 
                from_frame='ee_frame',to_frame='ee_frame'))
            ...
            self._post_pick_up_transforms[env_index] = gymapi.Transform(
                p=self._push_transforms[env_index].p + np_to_vec3([0.0, x_y_z_theta_dist[2], 0.0]),
                r=self._push_transforms[env_index].r
            )

            self._random_relative_transforms[env_index] = gymapi.Transform(
                p=self._post_pick_up_transforms[env_index].p + np_to_vec3(random_trans),
                r=self._post_pick_up_transforms[env_index].r * rpy_to_quat((x_y_z_rx_ry_rz[3], x_y_z_rx_ry_rz[5], -x_y_z_rx_ry_rz[4]))
            )
...
class PickUpBlockFlatWithFlipPolicy(Policy):
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 50:
            ...
            pre_pre_push_offset = [x_y_theta_dist[0], backoff_distance, 0.0]

            push_offset = [x_y_theta_dist[0], backoff_distance + displacement, 0.0]

            height = -0.03

            self._pre_pre_push_transforms[env_index] = RigidTransform_to_transform(transform_to_RigidTransform(gymapi.Transform(
                p=np_to_vec3(block_center) + np_to_vec3([0.0, 0.2, 0.0]),
                r=self._ee_transforms[env_index].r * rpy_to_quat((0, 0, x_y_theta_dist[2]))), 
                from_frame='ee_frame',to_frame='ee_frame') * RigidTransform(
                translation=pre_pre_push_offset, 
                from_frame='ee_frame',to_frame='ee_frame'))

            self._pre_push_transforms[env_index] = gymapi.Transform(
                p=self._pre_pre_push_transforms[env_index].p - np_to_vec3([0.0, 0.2-height, 0.0]),
                r=self._pre_pre_push_transforms[env_index].r
            )
            ...
            self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._pre_pre_push_transforms[env_index])

        if t_step == 100:
            self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._pre_push_transforms[env_index])

        if t_step >= 200 and t_step < 300:
            self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._push_interp_trajectories[env_index][t_step-200])

        if t_step >= 350 and t_step < 450:
            self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._pick_up_interp_trajectories[env_index][t_step-350])
...
class PickUpBlockTiltedPolicy(Policy):
    ...
    def __init__(self, franka, franka_name, block, block_name, n_envs, x_y_z_theta_tilt_dists, x_y_z_rx_ry_rz):
        ...
        self._time_horizon = 550

    def reset(self):
        self._ee_transforms = {}
        self._pre_push_transforms = {}
        self._push_transforms = {}
        self._post_push_transforms = {}
        self._random_relative_transforms = {}
    ...
    def __call__(self, scene, env_index, t_step, _):
        ...
        if t_step == 50:
            ...
            backoff_distance = -x_y_z_theta_tilt_dist[1] - 0.1*np.cos(np.abs(tilt_angle)) - 0.2

            pre_push_offset = [x_y_z_theta_tilt_dist[0], backoff_distance, 0.0]
            ...
            self._post_push_transforms[env_index] = gymapi.Transform(
                p=self._push_transforms[env_index].p + np_to_vec3([0.0, x_y_z_theta_tilt_dist[2], 0.0]),
                r=self._push_transforms[env_index].r
            )

            self._random_relative_transforms[env_index] = gymapi.Transform(
                p=self._post_push_transforms[env_index].p + np_to_vec3(random_trans),
                r=self._post_push_transforms[env_index].r * rpy_to_quat((x_y_z_rx_ry_rz[3], x_y_z_rx_ry_rz[5], -x_y_z_rx_ry_rz[4]))
            )
        ...
        if t_step == 250:
            self._franka.set_ee_transform(env_ptr, env_index, self._franka_name, self._post_push_transforms[env_index])
