...
if __name__ == "__main__":
    ...
    parser.add_argument('--spatula_flat_nn', type=str, default='same_block_data/spatula_flat_pick_up_only_trained_model.h5')
    parser.add_argument('--spatula_tilted_nn', type=str, default='same_block_data/spatula_tilted_pick_up_only_trained_model.h5')
    ...
    spatula_flat_nn_model = load_model(args.spatula_flat_nn)
    ...
    for suffix_idx1 in range(num_skills):
        for suffix_idx2 in range(suffix_idx1, num_skills):
            if (suffix_idx1 == suffix_idx2):
                ...
                contingency_nns[key+'_success'] = load_model(args.contingency_nn_dir + key + '_pick_up_only_success_contingency_model.h5')
                contingency_nns[key+'_failure'] = load_model(args.contingency_nn_dir + key + '_pick_up_only_failure_contingency_model.h5')
            else:
    ...
    spatula_flat_policy = PickUpBlockOnlyFlatPolicy(spatula_franka, 'franka2', block, 'block0', n_envs)
    spatula_tilted_policy = PickUpBlockOnlyTiltedPolicy(spatula_franka, 'franka2', block, 'block0', n_envs)
    ...
    for simulation_num in range(args.num_simulations):
        ...
        while skill_id != 0:
            ...
            current_input = resampled_points[random_num]
            skill_id = int(current_input[0])
        ...
        if skill_id == 0:
            ...
            spatula_flat_policy.set_x_y_theta_dist(current_input[1:])
            scene.run(time_horizon=spatula_flat_policy.time_horizon, policy=spatula_flat_policy, custom_draws=custom_draws, cb=(lambda scene, t_step, t_sim: save_block_pose(scene, t_step, t_sim, simulation_num=simulation_num, skill_id=skill_id)))
        ...
        if((post_pick_up_block_pose[simulation_num,1] - initial_block_pose[simulation_num,1]) > 0.1):
            ...
            fig = plt.figure()
            ax = plt.axes(projection='3d')
            p = ax.scatter3D(sorted_x_y_thetas[:,1], sorted_x_y_thetas[:,2], sorted_x_y_thetas[:,0], c=probabilities[:].flatten());
