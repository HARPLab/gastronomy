...
if __name__ == "__main__":
    ...
    skill_succeed_probabilities = np.ones(2000) * 0.5
    skill_failure_probabilities = np.ones(2000) * 0.5
    ...
    for current_block_id in range(101):
        ...
        initial_skill_succeed_probabilities = skill_succeed_probabilities.copy()#np.ones(2000)
        initial_skill_failure_probabilities = skill_failure_probabilities.copy()
        ...
        for i in range(args.num_trials):
            ...
            #random_skill_num = pick_random_skill_from_top_100(initial_skill_succeed_probabilities)
            ...
            initial_skill_probabilities /= (initial_skill_probabilities + skill_failures)
        ...
        predicted_successes = (initial_skill_probabilities/np.max(initial_skill_probabilities)) > 0.5
        print(predicted_successes)
        print(data[:,current_block_id] == 1)
        num_successes = f1_score(data[:,current_block_id] == 1, predicted_successes)
