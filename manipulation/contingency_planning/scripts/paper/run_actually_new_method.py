...
def pick_most_informative(skill_probabilities):
    argsort = np.argsort(skill_probabilities)
...
if __name__ == "__main__":
    ...
    initial_skill_success_probabilities = np.ones(num_skills_using) * 0.5
    initial_skill_failure_probabilities = np.ones(num_skills_using) * 0.5
    ...
    for current_block_id in range(101):
        ...
        for i in range(args.num_trials):
            ...
            #random_skill_num = pick_random_skill_from_top_100(skill_success_probabilities[:500])
            if i < 5:
                random_skill_num = pick_most_uncertain(skill_success_probabilities[:500])
            else:
                random_skill_num = pick_random_skill_from_top_100(skill_success_probabilities[:500])
