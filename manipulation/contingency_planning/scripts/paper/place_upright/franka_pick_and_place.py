...
from franka_policies import PickUpThenFlipPolicy
from get_pan_pose import get_pan_pose
from random_utils import *

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--franka_cfg', '-fc', type=str, default='cfg/paper/place_upright/franka_fingers.yaml')
    ...
    parser.add_argument('--original_inputs', '-oi', type=str, default='same_block_data/successful_lift_inputs_pick_up_only_with_franka_fingers.npy')
    ...
    parser.add_argument('--data_dir', '-dd', type=str, default='same_block_data/place_upright/')
    ...
    if args.inputs is None:
        if args.original_inputs is None:
            ...
            rotations = get_random_rotation_with_3_choices(num_iterations) 
            x_y_theta_rots = np.hstack((x_y_thetas,rotations)) 
        else:
            x_y_thetas = np.load(args.original_inputs)
            num_simulations = int(np.floor(x_y_thetas.shape[0] / n_envs))

            num_iterations = num_simulations * n_envs
            x_y_thetas = x_y_thetas[:num_iterations]
            x_y_theta_rots = np.vstack((np.hstack((x_y_thetas,np.ones((num_iterations,1))*np.pi/2)),
                                        np.hstack((x_y_thetas,np.zeros((num_iterations,1)))),
                                        np.hstack((x_y_thetas,np.ones((num_iterations,1))*-np.pi/2))))
            num_simulations *= 3
            num_iterations *= 3

    else:
        x_y_theta_rots = np.load(args.inputs)
        num_simulations = int(np.floor(x_y_theta_rots.shape[0] / n_envs))

        num_iterations = num_simulations * n_envs
        x_y_theta_rots = x_y_theta_rots[:num_iterations]

    policy = PickUpThenFlipPolicy(franka, 'franka0', block, 'block0', n_envs, x_y_theta_rots)
    ...
    np.savez(save_file_name, x_y_theta_rots=x_y_theta_rots, 
