...
from block_policies_with_tongs import PushBlockPolicy
from get_pan_pose import get_pan_pose

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg', '-c', type=str, default='cfg/push_block_with_tongs.yaml')
    ...
    while True:        
        ...
        for i, env_ptr in enumerate(scene.env_ptrs):
            ...
            if pre_push_z_offset > 0:
                ...
                pre_push_z_offset -= 0.05
                push_z_offset = -pre_push_z_offset
            env_offsets[i]['pre_push_offset'] = np_to_vec3([(np.random.rand()*2 - 1) * 0.05, 0.15, pre_push_z_offset])
            env_offsets[i]['push_offset'] = np_to_vec3([(np.random.rand()*2 - 1) * 0.05, 0.15, push_z_offset])
        policy.reset()
