...
from tong_policies import PickUpBlockTongsSidePolicy
...
if __name__ == "__main__":
    ...
    parser.add_argument('--block_cfg_dir', '-bcd', type=str, default='cfg/paper/baseline_same_friction/')
    parser.add_argument('--block_urdf_dir', '-bud', type=str, default='assets/baseline_same_friction/')
    ...
    parser.add_argument('--inputs', '-i', type=str, default='data/tongs_side_inputs.npy')
    parser.add_argument('--grasp_nn', type=str) #, default='baseline_same_friction/pick_up/tongs_side_pick_up_only_trained_model.h5')
    ...
    parser.add_argument('--data_dir', '-dd', type=str, default='baseline_same_friction/pick_up/tongs_side/')
    ...
    table2_pose = RigidTransform_to_transform(RigidTransform(
        translation=[table_width/3, table_height/2-0.01, 0]
    ))
    ...
    scene.add_asset('table1', table, table_pose, collision_filter=1)
    ...
    if args.inputs is None:
        if args.grasp_nn is None:
            ...
            probabilities = grasp_nn_model.predict(x_y_theta_dist_tilts)
            sorted_idx = np.argsort(-probabilities, 0)
            sorted_x_y_theta_dist_tilts = x_y_theta_dist_tilts[sorted_idx].reshape(-1,3)
            x_y_theta_dist_tilts = sorted_x_y_theta_dist_tilts[:args.num_iterations,:]
            np.save('same_blocks/pick_up/tongs_side_inputs.npy', x_y_theta_dist_tilts)
    else:
        x_y_theta_dist_tilts = np.load(args.inputs)
        
        num_simulations = int(np.floor(x_y_theta_dist_tilts.shape[0] / n_envs))

        num_iterations = num_simulations * n_envs
        x_y_theta_dist_tilts = x_y_theta_dist_tilts[:num_iterations]

    policy = PickUpBlockTongsSidePolicy(franka, 'franka0', block, 'block0', n_envs, x_y_theta_dist_tilts)
    ...
    save_file_name = args.data_dir+'block'+str(args.block_num)+'_pick_up_block_with_tongs_side.npz'
    np.savez(save_file_name, x_y_theta_dist_tilts=x_y_theta_dist_tilts, 
