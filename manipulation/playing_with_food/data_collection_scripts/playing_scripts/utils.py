...
def save_audio(dir_path, filename):
    cmd = "python data_collection_scripts/playing_scripts/sound_subscriber.py " + dir_path + filename 
    audio_p = subprocess.Popen("exec " + cmd, stdout=subprocess.PIPE, shell=True)
    return audio_p

def save_finger_vision(dir_path, filename):
    cmd = "python data_collection_scripts/playing_scripts/finger_vision_subscriber.py " + dir_path + filename 
    finger_vision_p = subprocess.Popen("exec " + cmd, stdout=subprocess.PIPE, shell=True)
    return finger_vision_p

def save_realsense(dir_path, filename, topic='/camera'):
    cmd = "python data_collection_scripts/playing_scripts/realsense_subscriber.py " + dir_path + filename + ' -t ' + topic
