...
if __name__ == '__main__':
    ...
    if args.slice_num > 0 and args.slice_num < 9:
        ...
        fa.goto_pose_delta(positive_delta_y_position, duration=10, use_impedance=False, force_thresholds=[10.0, 4.2, 10.0, 10.0, 10.0, 10.0])

        move_backwards_y_slightly = RigidTransform(translation=np.array([0.0, -0.009, 0.0]),
        from_frame='world', to_frame='world')   
        ...
        if args.slice_num == 1:
            ...
            slice_thickness = 0.008 #0.0 for 30 deg
        elif args.slice_num == 7:
            slice_thickness = 0.02
        elif args.slice_num == 8:
            slice_thickness = 0.03 #0.03 #0.01 for tomato
    ...
    dmp_num = 0
    while current_ht > 0.028: # 0.025 is cutting board ht    
