...
trials_with_no_object_in_ending_image = np.nonzero(object_position_changes[:,0].flatten() == -500)
print(trials_with_no_object_in_ending_image)
trials_with_object_in_ending_image = np.nonzero(object_position_changes[:,0].flatten() > -500)
...
distances = distance.pdist(object_position_changes)
...
for trial_idx in trials_with_object_in_ending_image[0]:
	...
	#print(correct_trial_distances.shape)
...
pick = open("positive_negative_labels_using_change_in_object_position.pkl",'wb')
