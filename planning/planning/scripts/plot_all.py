...
class Plot_All():
	...
	def plot_statistics(self):		
		...
		for simple in self.simples:
			...
			if not res_simple.empty:
				for horizon in self.horizons:
					horizon = int(horizon)
					res_horizon = res_simple.loc[res_simple['horizon'] == horizon]

					if result_1 is not None and not res_1_simple.empty:
						res_1_horizon = res_1_simple.loc[res_1_simple['horizon'] == horizon]

					if not res_horizon.empty:	
						if not self.compare_3T:
							...
							ax = self.plot_tables_planning_time_curve(res_horizon, res_1_horizon, simple, horizon, x, y, hue, execs)
	...
	def plot_tables_planning_time_curve(self, res, res_1, simple, horizon, x, y, hue, execs):
		print ("----- ", y)
		data = res[['id',x,y,hue]]
		if not self.compare_3T:
